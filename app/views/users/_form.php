...
<div class="line-form "> 
    <label class="label-form"> 
        <?php echo I18n("user_status_id"); ?> : <span class="mandatory ">*</span>
    </label> 
    <div class="content-input-form" > 
        <select name="user[user_status_id]" id="input-user-status" class="ui-widget-content ui-corner-all js-form js-mandatory js-show-view " > 
            <option value=""><?php echo I18n("select_user_status_id"); ?></option> 
            <?php
            $user_statuss = $user->get_user_status()->all();
            if ($user_statuss) {
                foreach ($user_statuss as $user_status) {
                    ?> 
                    <option <?php if ($user_status->get_id() == $user->get_user_status_id()) {
                        echo "selected='selected'";
                    } ?> value="<?php echo $user_status->get_id(); ?>"><?php echo $user_status->get_status($encoding); ?></option> 
    <?php } ?> 
<?php } ?> 
        </select> 
    </div> 
    <div class="content-error-form ui-state-highlight ui-corner-all hidden"> 
        <span class="icon-error-form ui-icon ui-icon-alert"></span> 
        <div class="error-form hidden" id="error-not-null-user-status"><?php echo I18n("error_not_null"); ?></div> 
    </div> 
</div> 
